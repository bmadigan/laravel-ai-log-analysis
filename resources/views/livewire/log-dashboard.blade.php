    ...
    <div class="py-12">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <div class="mb-6 flex justify-between items-center">
                <h1 class="text-3xl font-bold text-gray-900">Log Analysis Dashboard</h1>
                <button
            ...
            <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
                    ...
                    <div wire:loading.remove class="space-y-4">
                        @forelse ($logEntries as $entry)
                            <div class="border-l-4 border-gray-300 pl-4 py-2">
                                <p class="text-sm font-mono text-gray-700 break-all">{{ $entry['raw'] }}</p>
                ...
                <!-- Incidents Panel -->
                <div class="bg-white rounded-lg shadow-md p-6">
                    <h2 class="text-xl font-semibold text-gray-800 mb-4">AI-Detected Incidents</h2>
                        ...
                        @forelse ($incidents as $incident)
                            <div class="border-l-4 {{ $this->getSeverityBorderClass($incident['severity']) }} pl-4 py-2">
                                <div class="flex items-center gap-2 mb-1">
                                    ...
                                    <span class="text-xs text-gray-500">{{ $incident['created_at'] }}</span>
                                </div>
                                <p class="text-sm text-gray-800 font-medium">{{ $incident['summary'] }}</p>
                                <p class="text-xs text-gray-600 mt-1 font-mono">{{ $incident['log_preview'] }}</p>
                            </div>
                        @empty
                            <p class="text-gray-500 text-center py-8">No incidents detected yet.</p>
                        @endforelse
                    </div>
                </div>
            </div>
